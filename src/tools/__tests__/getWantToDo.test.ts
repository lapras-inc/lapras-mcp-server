...
import { GetWantToDoTool } from "../getWantToDo.js";
...
describe("GetWantToDoTool", () => {
  let tool: GetWantToDoTool;
  let mockFetch: ReturnType<typeof vi.fn>;
  const originalEnv = process.env;

  beforeEach(() => {
    tool = new GetWantToDoTool();
  ...
  it("今後のキャリアでやりたいことを正常に取得できる", async () => {
    const mockData = {
      want_to_do: "これからはAIエンジニアとして活躍したいです。",
    };
  ...
  it("APIリクエストが失敗した場合はエラーを返す", async () => {
    ...
    const result = await tool.execute();
    expect(result.isError).toBe(true);
    expect(result.content[0].text).toContain("今後のキャリアでやりたいことの取得に失敗しました");
